#include "TeleoperationDemoGripperSmall.h"
#include <mc_mori/devices/Led.h>
#include <mc_mori/devices/Debug.h>

using Color = mc_rtc::gui::Color;

TeleoperationDemoGripperSmall::TeleoperationDemoGripperSmall(mc_rbdyn::RobotModulePtr rm, double dt, const mc_rtc::Configuration & config)
: mc_control::fsm::Controller(rm, dt, config)
{
  ...
  mc_rtc::log::success("TeleoperationDemoGripperSmall init done");
}

bool TeleoperationDemoGripperSmall::run()
{
  return mc_control::fsm::Controller::run(mc_solver::FeedbackType::Joints);
}

void TeleoperationDemoGripperSmall::reset(const mc_control::ControllerResetData & reset_data)
{
  ...
  {
    auto postureObjective = getPostureTask("mori_gripper_small")->posture();
